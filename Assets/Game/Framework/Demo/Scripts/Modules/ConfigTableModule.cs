...
{
    /// <summary>
    /// 进阶·配置表：讲 Luban 集成的构建期 / 运行期分界——表定义（XML）与数据（JSON / Excel）放仓库根 Configs/，
    ...
    {
        private const string GenerateMenu = "SSFramework/配置表构建/1. 生成配置代码 + 数据 (Luban)";
        private const string ProfileMenu = "SSFramework/配置表构建/配置 (Luban Profile)";
        private const string OpenConfigsMenu = "SSFramework/配置表构建/打开目录/表定义与数据 (Configs)";
        ...
        public override void Build(DemoModuleHost host)
        {
            ...
            host.AddNote("表定义（XML）与数据（JSON / Excel）放在仓库根 `Configs/` 下（Assets 外，Unity 不导入）；构建期菜单跑 Luban CLI，" +
            ...
            host.AddStep("①", "改数据：`Configs/Datas/item.json`（JSON，diff 可读）或 `monster.xlsx`（Excel/WPS 直接编辑）；改表结构/加表：`Configs/Defines/demo.xml`。");
            host.AddStep("②", "菜单「SSFramework/配置表构建/1. 生成」：代码 / 数据 / 清单一次刷新（Play 中会被拒绝，先停）。");
            host.AddStep("③", "重新 Play 查看——配置在启动时一次性加载（只读数据不做运行中增量更新）。");
            host.AddActionRow("打开表定义与数据目录（Configs/）", () => RunMenu(OpenConfigsMenu));
            host.AddActionRow("生成配置代码 + 数据（真实跑 Luban CLI）", () => RunMenu(GenerateMenu));
            host.AddActionRow("定位生成配置（LubanConfigProfile）", () => RunMenu(ProfileMenu),
                new CodeRef("Assets/Game/Framework/Config/Editor/LubanConfigProfile.cs", "class LubanConfigProfile", "路径与目标的单一真源"));
