...
{
    /// <summary>
    /// System 层的游戏流程状态机。把「游戏宏观阶段」（启动 / 登录 / 大厅 / 战斗……）显式化为 <see cref="FlowState"/> 子类，
    ...
    /// 异常从 GoTo 的 task 冒出（调用方决定重试 / 进错误状态，框架不猜）。只有 flow 自己请求的顶替 / 销毁取消
    /// 才返回取消；下游操作在 flow token 未取消时自行抛 <see cref="System.OperationCanceledException"/> 会作为进入失败冒出，
    /// 避免 fire-and-forget Adapter 把真实故障误当正常顶替吞掉。<br/>
    ...
    public interface IGameFlow : ISystem
    {
        ...
        /// <c>OnEnter</c> 失败时携带其异常；非 flow 意图的自发取消同样归入失败。调用方必须 <c>await</c> 或显式观察三种结局；
