...
namespace Game.Framework
{
    ...
    /// </para>
    /// <para><b>线程边界：</b>异步 Provider 方法，以及返回的 downloader / scene handle 的异步方法，
    /// 都允许在任意线程物理完成；不得假设调用方 continuation 所在线程。
    /// <see cref="AssetUtility"/> 会在修改包状态、推进维护队列、处理 Unity handle 或完成公共 task 前恢复 Unity 主线程，
    /// 并为业务拿到的 downloader / scene handle 包一层相同的异步终态边界。同步查询、下载器进度通知与返回 handle 的
    /// 同步操作仍由 Provider 按各成员文档的主线程契约实现。</para>
    /// </summary>
