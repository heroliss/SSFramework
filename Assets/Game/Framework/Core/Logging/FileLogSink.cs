...
namespace Game.Framework.Logging
{
    ...
    /// </summary>
    /// <remarks>
    /// 用法：<c>FrameworkLog.AddSink(new FileLogSink(Path.Combine(Application.persistentDataPath, "logs", "framework.log")))</c>；
    /// 不再需要时 <see cref="Dispose"/>（或 <c>FrameworkLog.RemoveSink</c> 后 Dispose）关闭句柄。<br/>
    ...
    public sealed class FileLogSink : ILogSink, IDisposable
    {
        ...
        private static string Format(in LogEntry entry)
        {
            ...
            return entry.Exception != null ? line + Environment.NewLine + entry.Exception : line;
        }
