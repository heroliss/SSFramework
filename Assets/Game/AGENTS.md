...
- ⚠ **推送事件类型实现 `IEvent`**（`RegisterPush<TEvent>` 约束就是 `IEvent`）：默认 JsonUtility 序列化器下用 `[Serializable] struct + 公共字段`（只认字段，**不能用 record 位置参数**——那是属性、反序列化不出来）；**二进制序列化器（Protobuf 等）可用 class 消息**（如 protoc 生成的 `IMessage`），但**引用类型事件必须带 payload**（空 payload 无默认实例可发，会被丢弃告警；struct 空 payload 仍取 `default`）。请求 / 响应类型是 `[Serializable] class`（或生成的 `IMessage`）。
