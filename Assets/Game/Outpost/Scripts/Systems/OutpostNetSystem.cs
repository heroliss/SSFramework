...
{
    /// <summary>
    /// 排行榜长连接的维持者：启动时连 dev server 的 WS、注册「全服纪录刷新」推送并 Toast 提示，
    ...
    /// </summary>
    /// <remarks>
    /// 网络栈仅在 Editor / Development Build 注册（M4 的对端是进程内 <c>Net.OutpostDevServer</c>），
    /// 正式包本系统空转（<c>OutpostNet.Available</c> 为 false，实现整体条件编译）。
    /// </remarks>
    public sealed class OutpostNetSystem : MonoSystemBase
    {
#if UNITY_EDITOR || DEVELOPMENT_BUILD
        private const int MaxBackoffExponent = 5; // 退避 1,2,4,8,16,32s 封顶——dev server 就在本进程，没必要更长
        ...
        private async UniTaskVoid MaintainConnection()
        {
            ...
            {
                var ws = this.GetUtility<IWebSocketUtility>();
                string url = this.GetUtility<OutpostDevServer>().WsUrl;
                for (int attempt = 0; ; attempt++)
                {
                    ...
                    {
                        // 连接失败 → 退避后再试。dev server 与游戏同进程，常态是首连即成；
                        // 失败重试路径主要为验证样板本身（真后端下这里还会区分可重试与否）。
                    }
