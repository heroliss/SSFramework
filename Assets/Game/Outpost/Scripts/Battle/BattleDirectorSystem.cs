...
namespace Game.Outpost.Battle
{
    ...
    public sealed class BattleDirectorSystem : MonoSystemBase
    {
        ...
        // 离散合爆全额让位给连续怒吼。18 杀/秒以下 blend=0（此刻爆炸稀疏、逐记可辨=离散合爆就是
        // 正确听感），60 杀/秒以上满接棒（这已是"数不清的连续爆炸"，任何离散"咚"都不该冒头）。
        // 阈值从 25~100 收到 18~60（第五轮）：波次爬坡/收尾的中等击杀率也该尽早交给怒吼——
        // 那正是"怒吼弱 + 裸合爆突出"的暴露区（配套底床衰减放慢、间隙里延续盖住零星残响）。
        private float KillFusionBlend() => Mathf.Clamp01((_killRate - 18f) / 42f);
        ...
        private void TryFlushBoom(float now)
        {
            ...
            // 接棒**全额**让位（与单发层→火墙同构，音量随 blend 归零）：部分衰减试了三档（75%/55%/68%）
            // 都被听出"咚咚"——人耳对瞬态的探测不看音量，离散炸点压多小声都能被清晰追踪；物理上
            // 每秒几百爆的合并结果就是一整段连续轰鸣，任何离散"咚"都不该存在（用户点破）。融合区
            // 门控不播、不清扇区、不耗冷却：能量自然衰减，击杀率回落 blend 松开时残余合爆立即复现。
            float blend = KillFusionBlend();
            float volume = Mathf.Min(Mathf.Sqrt(bestE), 0.95f) * (1f - blend);
            ...
            pitch *= 1f - 0.12f * Mathf.Clamp01((bestE - 0.5f) / 2f) * (1f - blend); // 体量下沉随接棒退坡
        ...
        private void UpdateKillAudio(float dt)
        {
            ...
            // 平滑击杀率：上升快下降**慢**——轰鸣的余韵比爆发慢很多拍，读作战场回响而不是开关。
            // 下降 k 从 2.5 降到 1.2（余韵 ~0.8s→~1.5s）：波次收尾/间隙里怒吼延续下来盖住零星
            // 裸合爆（第五轮"单个击毁咚咚突出"的一半病根是怒吼在间隙消失、裸露了残敌的离散合爆）。
            float inst = dt > 0f ? _killsThisFrame / dt : 0f;
            _killsThisFrame = 0;
            float k = inst > _killRate ? 6f : 1.2f;
