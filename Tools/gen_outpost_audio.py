...
def make_kill_rumble():
    # 战场轰鸣底床（4s 无缝循环）：海量击杀融成的**连续怒吼**。
    #
    # 合成模型（第四轮试听推翻前三轮的"瞬态求和"）：大量爆炸在高速率下的极限 = 滤波噪声。
    # 这是中心极限定理——N 个随机时刻的冲击求和，N→大 时收敛到服从单个冲击频谱包络的高斯
    # 噪声。连续炮火的"怒吼"物理本质就是爆炸频谱塑形的**稳态噪声**，不是一串可数的爆炸声。
    # 前三轮走"逐记空爆瞬态随机叠加"：无论密度堆到多高（实测 300 记/秒）、无论逐记起音怎么
    # 抹平，低频带（<300Hz）的起音陡度都卡在 0.63（稳态噪声地板 0.35），即残留可辨的"咚咚"
    # ——因为 30~165Hz 冲击体是瞬时起音，低通滤不掉时域起音，人耳对瞬态的探测**不看音量**
...
def make_fire_gears():
    ...
    for rate in FIRE_GEAR_RATES:
        ...
        if fused:
            ...
            # 0.32/0.68 + 1.8k：首调 0.45/0.55+2k 试听反馈"高频射击声偏大"——饱和段还要 ×1.5 变速
            # 把频谱再抬 50%，顶档资产得为此预留暗度。
            if rate >= 256.0:
                x = 0.32 * x + 0.68 * lowpass(x, 1800, order=1)
