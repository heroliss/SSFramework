...
$frameworkGitUrl = 'https://github.com/heroliss/SSFramework.git#v0.1.1'
Write-Host ''
...
elseif ($addedFramework) { Write-Host '  SSFramework：将发布标签 v0.1.1 加入清单，Unity 启动后自动下载。' }
...
if ($CheckNetwork -and -not $SkipNetworkCheck -and $manifestChanged) {
    ...
    if ($addedFramework) { $plan.NetworkChecks += Test-SetupEndpoint 'SSFramework / GitHub' 'https://raw.githubusercontent.com/heroliss/SSFramework/v0.1.1/package.json' 'com.liss.ssframework' }
