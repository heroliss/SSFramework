...
| 测试 | PlayMode 525 + EditMode 363，共 888 项全绿；交互式 MCP 后台运行且 PlayMode 先预检，命令行入口默认 EditMode + PlayMode |
...
- Context / DI / Pool 的异常、警告与 Trace 改为中文动作优先，同时保留 `Context`、类型名、`GetModel`、`IDisposable`、`Spawn/Despawn` 等可复制标识；覆盖容器构建与所有权回滚、Mono Context 初始化、注入权限、主线程守卫、Bag 归还和对象池误用。相关测试不再锁整句英文，而是断言“中文语义 + 动态类型/API + 原始异常”；Unity 编译 0 错误/0 警告，完整 EditMode 363/363、PlayMode 525/525 通过。
...
| P1 | 中文友好性继续收口 | 高频 Inspector、诊断状态、资源/配置/YooAsset、Context/DI/Pool/Bag 运行时反馈，以及 Demo 入门术语、日志入口和资源流程层级已完成；下一步收敛 Lifecycle / Flow / UI Toolkit / Audio 等剩余低频英文异常与 Profile。始终保留类型、参数和第三方原始错误供检索，不引入过度的运行时多语言系统。 |
