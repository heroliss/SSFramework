...
- **proto 生产化**（官方 protoc + Google.Protobuf 适配器，接缝已就位）——**待定**：手写 ProtobufNetworkSerializer 已工作且经服务端标准 protobuf 客户端反证是真 wire 格式；生产化需往 Unity 装 Google.Protobuf + protoc（提交二进制 + HybridCLR AOT 顾虑），路径待用户拍板。
