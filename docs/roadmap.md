...
5. **交互式 Editor 的 AI 测试预检** ✅ 已落地（ADR-0036）：MCP 跑 PlayMode 前显式保存已有路径脏场景，未命名场景按整批先验证再写入的顺序 fail-fast，避免原生保存弹窗锁死 Unity 主线程队列；不注册全局自动保存，不改变人工 Play 语义。Outpost 真实玩家路径冒烟同步落地，以原子目录重命名保护真实存档，并修出 Context 隔离、收尾状态与 Test Framework 协程续跑等组合缺陷；当前基线 PlayMode 422/422 + EditMode 106/106。
