...
MVCS 经 Command 的写链（简单操作 `View → Command → Model`，规则型操作 `View → Command → System → Model`）天然适合**离散的、用户/网络意图驱动**的状态变更（背包、商店、对话、回合制操作）。但游戏还有大量**连续的、逐帧推进的仿真**：AI tick、移动/物理、技能结算、寻路、计时器、状态机轮询。

此前文档对这一半着墨偏薄，留下歧义：逐帧逻辑该住在哪？谁驱动每帧？要不要也包成 Command？若误把逐帧逻辑包成 Command 每帧 `Execute`，会有两个坏处：

1. 每帧产生无谓的命令分发（即便 struct 零分配，单泛型查询重载仍可能装箱，见[用户手册「带返回值的 Command」](../framework-guide.md#带返回值的-command)）；
