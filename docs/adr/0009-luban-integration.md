...
- 表定义（XML）与数据（JSON / Excel）放各自的 conf 源目录；`~` 后缀可用于阻止 Unity 导入构建期输入。Editor 工作台 `SSFramework/代码生成/配置表 (Luban)` 封装 Luban CLI。路径与目标收口在 `LubanConfigProfile`（每套配置一份 SO），生成逻辑在 `LubanCodeGenerator`（均属 `Game.Framework.Config.Editor`）。工程可并存多套 profile（多套配置），`ResolveAll()` 返回全部、逐套生成。
...
- 输出格式定 **binary**（紧凑、解析快）；数据源按表选、同项目混搭（表定义的 `input` 决定）——示例工程双样例：`item.json`（JSON 文本，git diff 可读、AI 可维护）+ `monster.xlsx`（Excel，策划直接编辑）。
...
- `Game.Framework.Config`：引用热更内核 → **在热更列表**（0008 铁律自动校验通过，拓扑序 Framework → Asset.Yoo → Config → 业务程序集）。
- 生成代码：归业务程序集（示例工程归 `示例程序集`，editor-only）；业务热更则生成代码天然在热更侧。

## Consequences

- ✅ 配置加载复用 `IAssetUtility`（自加载 Utility 直接 `GetUtility` 取它，靠 `IUtility : ICanGetUtility`），与资源系统同一套初始化/多包/CDN/热更机制。
- ✅ 框架模块后端无关：换 JSON / 自定义格式只换项目侧工厂；`src/Config/` 可整目录删除，框架其余零感知。
...
- ⚠ 生成代码 namespace（topModule）**不得嵌进含 `System` 子命名空间的层级**（如 `Game.Framework.*`）：生成代码裸写 `System.Func` 会被就近解析劫持（CS0234）。示例使用顶层 `AppCfg`。
...
用法手册见 `docs/framework-guide.md` §16；接入时以项目自己的配置源目录、生成产物和测试作为验证依据。