...
当前档位、Profile key、子进程 PID、子进程 UTC 启动时间、待运行队列与“当前完成后停止”的具体原因持续写入 `report.json`；运行目录由本机 EditorPrefs 保存，输出 / 结果 / 日志路径在读取时按 Profile key 重建，不进入分享 JSON。主报告与 child 独立结果都先写同目录临时文件，再以替换/移动发布；`report.json` 最后发布并作为恢复提交标记，写入中断只能留下上一份完整 JSON，不能留下半截新状态。启动新一轮时先写本机 previous-latest owner journal，再发布首代 JSON，最后才切换 latest 指针，因此任一中断点都至少保留一份可读取的 latest；最终状态无法持久化时恢复上一指针并清理 running 状态。已启动 child 则继续由原 owner 观察到终态，停止后续档位，不能覆盖进程句柄或让 Domain Reload 复活已收口任务。

主 Unity 发生 Domain Reload 或重启后，只在 PID、UTC 启动时间与 Unity 进程名全部匹配，且候选不是当前主 Editor 自身时重新附着 child；这避免 PID 复用或错误自附着。进程检查因权限或平台异常而无法证明身份，与 PID / 启动时间尚未提交一样都属于“所有者未知”，必须 fail-closed 停止整个矩阵，不能启动另一个 Unity 并发写同一隔离工程。若独立结果已经原子落盘，仍保留当前档真实成功/失败结论，但用独立 owner 原因跳过后续档位；结果不存在则当前档明确失败。恢复只重建本报告实际记录的 Profile，不计算无关的完整 Module 假设矩阵。停止原因是报告状态而不是易失的静态布尔值：人工请求和自动证据漂移在重载后都会继续生效，最终跳过项也保留真实原因。若恢复时恰逢 manifest / Package / Source Catalog 正在变化，连当前拓扑都无法重建，该异常也被归为漂移：优先按已验证的 child 身份附着当前进程，完成后停止；child 已退出但冻结输入产生的独立结果已落盘时，先消费该结果再停止后续档位；只有进程和结果都不存在时才明确失败并完成报告，不让子进程因主 Editor 的重建异常失去 owner，也不用空数据猜成功。报告格式 v9 将上述 child 身份字段纳入恢复契约；更早格式不会被当前实现猜测续跑。
...
预览缓存不是构建输入。点击构建或调用无窗口机器菜单时，动作 Implementation 必须重新采集当前审计证据，并且只为请求的 Profile 计算闭包、manifest 与内容指纹；不能为了方便复用可能陈旧的窗口对象，也不应先为未选组合做昂贵哈希。共享缓存刷新会同时更新所有已打开的审计/体积窗口，失败则让旧视图统一进入失效态；观察者异常按订阅者隔离。体积窗口把所选 Profile 作为独立于 Toggle 是否已创建的意图保存，因此进阶区折叠、键盘展开或执行前刷新都不会悄悄重置选择。这样把“打开工作台”“理解并选择”“冻结真实执行证据”分成三个明确阶段，同时保留启动时 fail-fast 与整轮漂移检测。
