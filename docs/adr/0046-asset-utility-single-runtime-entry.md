...
- 三个组件必须同 Context、依赖执行顺序与 `[Inject]` 才能工作，Inspector 与 示例工程 需要反复解释“为什么少一个就坏”，增加接线错误和教学负担。
...
2. `AssetRuntimeSettings` 是普通 `[Serializable]` 基础设施配置，不实现 `IModel` / `ISystem`，也不注册到 Container。业务继续只依赖 `IAssetUtility`；需要展示 Inspector 场景创作配置的教学/Editor 代码从具体 `AssetUtility.Settings` 读取。序列化字段仍用 Unity 可编辑的 `List<T>`，公开集合只返回缓存的结构只读视图，不泄漏可强转修改的内部列表。
3. 场景路径在 `AssetUtility.Awake` 应用配置并建立状态，在 `Start` 执行自动初始化。这样 `AddComponent<AssetUtility>()` 后立即调用 `Configure` 的代码引导拥有一个确定窗口；显式 `Configure` 会接管启动并抑制 Inspector 自动初始化。`Settings` 始终表示 Inspector 场景创作配置；`Configure` 提交独立运行快照而不回写它，并深拷贝调用方 `AssetProviderConfig`，每次初始化再给 Provider Adapter 独立快照。调用方或 Adapter 后续修改自己的 DTO 都不会形成隐式热换。
...
7. 教学场景与真实消费工程游戏通过 Unity Editor 迁移为单组件入口。示例工程、主指南、资源流程与加密文档统一把 `AssetUtility.Settings` 称为场景创作配置，并把代码路径的当前生效值称为 `Configure` 运行快照。
...
- 依赖把 `Settings.Packages/CdnUrls` 强转为 `List<T>` 后修改，或在 `Configure` 后继续修改原 DTO 的旧代码不再生效；这些路径从未是正式 Interface，迁移方式是在 Play 前编辑 Inspector，或在 Start 前重新组装并提交配置。
...
- 运行配置测试锁定 Settings 集合不可结构修改、缓存视图在 Unity 替换序列化列表后不会陈旧，并验证调用方 DTO 与 Provider DTO 都不能反向改写 Utility-owned 快照。
...
- 示例场景与消费工程场景迁移前后逐字段 diff 相同，且场景中不再存在旧配置/初始化组件。
