...
- 三个组件必须同 Context、依赖执行顺序与 `[Inject]` 才能工作，Inspector 与 教学工程 需要反复解释“为什么少一个就坏”，增加接线错误和教学负担。
...
7. 教学场景与真实消费方游戏通过 Unity Editor 迁移为单组件入口。教学工程、主指南、资源流程与加密文档统一把 `AssetUtility.Settings` 称为场景创作配置，并把代码路径的当前生效值称为 `Configure` 运行快照。
