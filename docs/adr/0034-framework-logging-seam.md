...
- Audio Runtime 的淡变/回收驱动异常与 Dispose 后误用已迁入 Seam，异步异常保留 exception 和可用的 Unity context。AOT `Game.Framework.Boot` 则明确保留原生 `Debug.*`：它在框架与热更程序集加载前自举，asmdef 刻意不引用 `Game.Framework`；为统一写法反向依赖 Core 会破坏 Boot Module 的隔离，另造一套启动日志门面也没有 Leverage。
