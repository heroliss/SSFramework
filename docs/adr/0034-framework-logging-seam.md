...
- **依赖引入分界**：接缝 + 两个内核 sink（本 ADR 的阶段 A）零第三方依赖、可立即落地；ZLogger 模块（阶段 B）需经 NuGetForUnity 引入 ZLogger + `Microsoft.Extensions.Logging` 一串 DLL 并验证 IL2CPP / source generator，属于「新增第三方依赖」决策，实现该阶段前单独向用户确认（AGENTS 复用铁律：新增第三方依赖先加 asmdef references 再用）。

## 五件套落地

- ① ADR：本文。
- ② 接口在内核、实现在模块：`Core/Logging/`（门面 + `ILogSink` + 两个默认 sink）；`Game.Framework.Logging.ZLogger/`（可选 sink 模块）。
