...
- 应用 / 还原后对使用主字体的存活 TMP 文本 `ForceMeshUpdate(true, true)` 强刷（TMP 有字形解析缓存）；Toolkit 文本随 locale 切换本就重设文本（`BindLocalizedText`），固定文本 + 链条变化的罕见场景由业务重设 text 触发重排（示例中有样板）。
...
- 内核不需要接口：Framework Core 不会反向调用字体链，`MonoLocaleFonts` 自己拥有 locale 订阅、fallback 合成与资源释放这组完整行为；为“每个 Module 都要一对 Interface / Implementation”而造空接缝没有删除价值。顺带移除内核 asmdef 里无使用者的 `Unity.TextMeshPro` 引用（TMP 依赖归本模块）。
...
- batchmode 无字体渲染可言：测试覆盖「链条写入 / locale 切换驱动 / 未配置降级 / OnDestroy 还原 / OS 候选择取」（运行时创建字体资产在 PlayMode 测试可行，Windows 机器用 Arial 做候选）；渲染效果靠独立运行时场景人工验证。
