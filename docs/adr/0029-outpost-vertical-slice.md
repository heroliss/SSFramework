...
3. **`BindLocalizedText` 的刷新信号只有 Locale**：文本源（配置表）异步后到不会触发重绑——业务解法 = Boot 在进 UI 前 await 配置 Ready or Failed（Failed 放行，裸 key 是可见的缺失报告）。框架侧是否让 source 就绪也成为信号，待有第二个受害者再议。
