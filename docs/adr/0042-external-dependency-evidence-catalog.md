...
6. 审计窗口保持只读。它提供来源定位、证据复制和渐进展开；Package 的安装、版本选择与卸载继续由 Unity Package Manager 负责，框架不调用 `Client.Add/Remove`。会话缓存的显式刷新会先失效旧快照，成功后广播同一份新 `Entry` 给所有已打开窗口；失败不回退成“仍然当前”的旧证据。每个观察者独立隔离，单个窗口异常不能阻止缓存提交或其它窗口更新。
...
- 首次冷扫描需要读取 Editor DLL 元数据；后续先流式计算内容 SHA-256，相同字节复用 AssemblyRef，只有内容变化才再次装入反射只读程序集。不能只用文件长度/写入时间命中缓存：保留时间戳的原地 DLL 替换会让旧引用证据跨刷新存活。若规模继续增长，应增量缓存证据输入，而不是删掉 Editor 消费层换取假快。
