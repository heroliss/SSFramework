...
但 Odin 是框架里**唯一没被隔离、却承重**的第三方依赖（YooAsset 等都关在接口后，Odin 却在每个 Mono 基类的继承链里），且它是 [ADR-0010](0010-framework-reusability-upm.md) 「UPM 抽包、广泛复用」愿景的**前置闸门**——复用方被迫购买 Odin。本 ADR 先把"到底怎么解耦、改动面多大、风险在哪"评估清楚，再决定是否排期。
...
- 🔮 **建议排期**：把 Odin 解耦定位为「UPM 抽包前必做的技术债」（[ADR-0010](0010-framework-reusability-upm.md) 的前置）。越晚做、依赖 Odin 序列化的场景数据越多、迁移越痛。落地时本 ADR 转 Accepted 并补迁移记录。
