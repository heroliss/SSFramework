...
Framework 提供字体链与字集生成机制，UPM 包本身不强制下载字体或创建默认 locale 配置。需要快速显示中文时，可另行导入[简体中文入门字体包](starter-fonts.md)，直接使用 TMP / TextCore 动态资产，或作为下文的 locale 补充字体；也可自行选择字体。字体导入后仍由消费工程显式接线与维护。
