...
`SSFramework-Archive` 已在 GitHub 设置为 archived/read-only。归档后的远端会拒绝 push，即使本地已经有新的 README 提交；这不是 Git 历史或权限配置错误。需要补充归档说明时，可以先在本地工作树形成提交并保留提交号，待仓库所有者在 GitHub 设置中临时取消归档后推送，再恢复归档状态。归档仓库不应重新加入任何消费仓库的 `.gitmodules`。
