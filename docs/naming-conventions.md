...
- **仓库名**使用产品或工程名：`SSFramework`、`Outpost`、`FrameworkTutorial`、`NomadWorkshop`。教程仓库的目标公开名称为 `SSFrameworkTutorial`，但当前 GitHub 远端仍为 `FrameworkTutorial`；仓库改名和 Unity 资产路径迁移分开执行。
...
- **FrameworkTutorial** 的公开仓库名已从 `DemoScene` 改为教程语义，后续可进一步改为 `SSFrameworkTutorial`。其 Unity 资产路径和 `Game.Framework.Demo` 程序集标识暂时保留，待通过 Unity Editor/MCP 完成序列化迁移后再改为 `Tutorial`，不手改场景或 Prefab YAML。
