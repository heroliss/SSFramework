...
默认单 Agent 直接完成。只有下面场景才切换执行方式，避免为简单任务增加协调成本。
...
满足上述边界时可自主启动，不必逐次等待用户批准；启动时立即说明 Agent 类型、只读/读写边界、预期产出和并行收益，让用户知道工作如何被拆分。用户明确要求单 Agent、任务只是局部修改、子任务会争写同一文件，或主任务需要持续交互澄清时不得启动。不要把交互式开发机械拆成 plan → impl → review 流水线。

非琐碎 feature 完成、改公共 API/架构、或准备合并时，可自主启动只读 reviewer 并告知用户。给 reviewer 代码与关注点，不灌输设计理由；关注边界条件、错误处理、命名一致性、测试缺口和本目录规则。
