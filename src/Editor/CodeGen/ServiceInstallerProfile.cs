...
namespace Game.Framework.Editor
{
    ...
    /// 每个条目必须独占一个位于 <c>Assets</c> 内的输出文件；生成入口会与其它生成器的目录、后缀和文件声明统一预检，防止覆盖或被清理。生成入口：
    ...
    public sealed class ServiceInstallerProfile : ScriptableObject
    {
        ...
        public sealed class InstallerEntry
        {
            ...
            [Tooltip("安装器输出路径：必须位于 Assets 子目录，且以 .cs 结尾（建议 .g.cs）。共享输出声明目录会拒绝重复文件或落入其它生成器的清理范围。类名 = 文件名去掉 .g.cs 后缀。新条目留空，需按目标业务程序集明确填写。")]
        ...
        public static IReadOnlyList<ServiceInstallerProfile> ResolveAll() =>
            FrameworkEditorProfileCatalog.ResolveAll<ServiceInstallerProfile>();
    }
}
