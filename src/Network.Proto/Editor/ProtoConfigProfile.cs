...
namespace Game.Framework.Network.Proto.Editor
{
    ...
    /// <b>工程可并存多套</b>（正式协议一套 + 框架测试一套等）：每套指向各自的 .proto 源目录与输出目录、
    /// 互不干扰；<see cref="ResolveAll"/> 返回全部、生成菜单逐套生成。
    ...
    public sealed class ProtoConfigProfile : ScriptableObject
    {
        ...
        [Header("产物输出")]
        [Tooltip("生成 C#（*.g.cs）的输出目录（相对工程根目录）。目录里的 *.g.cs 由生成器接管：\n" +
                 ".proto 改名 / 删除后遗留的陈旧 *.g.cs 会被自动清理，勿手放同后缀文件。")]
        ...
        /// 生成菜单逐套生成、互不干扰。一套都没有时返回空表（由菜单 / 总览窗口引导创建）。
        /// </summary>
