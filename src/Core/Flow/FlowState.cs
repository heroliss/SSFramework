...
namespace Game.Framework.Flow
{
    ...
    public abstract class FlowState
    {
        ...
        /// 仅 <see cref="OnEnter"/> / <see cref="OnExit"/> 期间可用，其余时机为 null。
        /// </summary>
        ...
        /// 仅 <see cref="OnEnter"/> / <see cref="OnExit"/> 期间可用。
        /// </summary>
            ...
            get
            {
                if (_scope == null)
                    throw new InvalidOperationException(
                        $"[FlowState] '{GetType().Name}' 的 Bag 仅在 OnEnter/OnExit 期间可用——InstallBindings 阶段子 Context 尚未构建。");
        ...
        /// 时<b>不保证</b>被调——可靠的清理写进 <see cref="Bag"/>，这里只放"优雅告别"类工作如存档上报）。
        /// 抛异常只记录不阻断转换（离开失败不该把游戏卡死在旧阶段）。
        /// </summary>
