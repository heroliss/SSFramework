...
namespace Game.Framework.Demo.Modules
{
    ...
    public sealed class CounterDemoModule : DemoModuleBase
    {
        ...
        public override void Build(DemoModuleHost host)
        {
            ...
            host.AddNote("Command 直接改 Model 是完整的最简形态，不是等待重构的过渡代码；当规则出现校验、多步协调或复用需求时，再抽到 System，见「逻辑系统（System）· 规则归位」。");
            ...
            host.AddConcept("耗时操作", "改用 `IAsyncCommand` 并透传取消令牌，让异步生命周期跟随 View / Context；见「命令（Command）· 三种形态」。");
        }
    }
