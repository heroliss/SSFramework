...
namespace Game.Framework.Pool
{
    ...
    /// 但仍不依赖框架 Context；位置加载交由调用方先 <c>Bag.Load&lt;GameObject&gt;(location)</c> 再建池，刻意不把 IAssetUtility 拉进来。<br/>
    ...
    public sealed class PoolUtility : IPoolUtility
    {
        ...
        public IGameObjectPool GetGameObjectPool(GameObject prefab, int maxSize = 0)
        {
            ...
            // 每个 prefab 一个命名子节点，便于在 Hierarchy 里观察各池的空闲实例。
            var parking = new GameObject(prefab.name).transform;
            parking.SetParent(EnsureParkingRoot(), false);
            var pool = new GameObjectPool(prefab, parking, maxSize);
